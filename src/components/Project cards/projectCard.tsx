...
interface ProjectCardProps {
  ...
  function ProjectCard({ reverse = false, image, title }: ProjectCardProps) {

    return (
        <div className={`flex flex-col gap-6 md:flex-row md:gap-40 ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} text-white `} >
            <div className="w-full h-[220px] bg-cover bg-center rounded-[8px] flex items-end pl-4 pb-2 md:w-1/2 md:h-96 md:pl-9 md:pb-4 group cursor-pointer" style={{ backgroundImage: `url("${image}")` }}>
                <button className='bg-[#FFFF]/25 size-9 flex justify-center items-center rounded-full md:size-16 group-hover:bg-white transition delay-150 duration-300 ease-in-out'>
            ...
            <div className="flex flex-col md:w-1/2 md:justify-between">
                <h1 className='text-[16px] proximaNovaBold uppercase mt-2 mb-1 md:text-[32px]'>
                    {title}
                </h1>
                <p className='hidden md:inline text-2xl proximaNova'>
